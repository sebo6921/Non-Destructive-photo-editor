 ...
 * </p>
 * <p>
 * A block averaging filter pixelates an image by replacing each pixel by the
 * average of the pixels in a surrounding neighbourhood
 * </p>
 * 
 * <p>
    ...
    public BufferedImage apply(BufferedImage input) {
        ...
        for (int y = 0; y < input.getHeight(); y += blockSize) {
            for (int x = 0; x < input.getWidth(); x += blockSize) {
                ...
                // Set the color of all pixels within the current block to the calculated
                // average color
                for (int blockY = y; blockY < y + blockSize && blockY < input.getHeight(); blockY++) {
                    for (int blockX = x; blockX < x + blockSize && blockX < input.getWidth(); blockX++) {
